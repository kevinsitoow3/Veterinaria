...
from models.models import Owner as OwnerModel
from schemas import Owner, OwnerCreate
...
@router.put("/{owner_id}")
def update_owner(owner_id: int, owner_update: OwnerCreate, db: Session = Depends(get_db)):
    owner = db.query(OwnerModel).filter(OwnerModel.id_dueño == owner_id).first()
    if not owner:
        raise HTTPException(status_code=404, detail="Dueño no encontrado")
